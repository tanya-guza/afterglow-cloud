...
def _logglyAuth(request):
    ...
    resp, content = client.request(request_token_url, method="POST",
                                   body=urllib.urlencode({'oauth_callback': settings.LOGGLY_OAUTH_CALLBACK}))
...
def receiveCallback(request):
    ...
    if "oauth_callback_confirmed" in request.GET and \
        ...
        response.set_cookie(key="afLoggly",
                            value=_buildLogglyCookie(access_token['oauth_token'], access_token['oauth_token_secret'],
                                                     request.session["logglySubdomain"]),
                            expires=expiry)
...
def logglySearch(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            params = "?q=%s&from=%s&until=%s&rows=%s&start=%s" % (
            urllib.quote(request.POST['query']), urllib.quote(request.POST['dateFrom']),
            urllib.quote(request.POST['dateUntil']), urllib.quote(request.POST['rows']),
            urllib.quote(request.POST['start']))
